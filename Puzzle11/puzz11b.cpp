...
/// @brief Simulate the "keep away" among the monkeys over 10'000 rounds
///
/// @param monkeys : The list of monkeys
/// @param multedFactors : The compounded multiplication of all monkeys'
///   divisibility factors

void simulate(std::vector<Monkey>& monkeys, const int multedFactors);
...
int main() {
    ...
    // Compounded multiplication of all monkeys' factors
    // Used to manage "worry level" so that items do not have 64-bit overflows
    int multedFactors = 1;
    ...
    // Run simulation
    simulate(monkeys, multedFactors);
