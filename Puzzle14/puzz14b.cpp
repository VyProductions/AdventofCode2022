...
/// @brief Simulate dropping sand until the sand spawner is blocked
/// @param spawnPos : Where to spawn the sand from
/// @param tileMap : The current tile grid content
/// @return how many tiles of sand were spawned until sand blocked the spawner
...
int main() {
    ...
    // How far horizontally sand can travel before the spawner is blocked
    int triangleBase = maxR + 2;
    // Create floor around spawn column
    drawLine(
        {maxR + 2, 500 - triangleBase},
        {maxR + 2, 500 + triangleBase},
        tileMap
    );

    // How much sand was dropped until sand blocked the spawner
...
int dropSand(
    ...
    while (!blocked) {
        ...
        while (!settleSand(sandPos, tileMap)) {}

        // If sand is not able to move from start point
        if (sandPos == spawnPos) {
            blocked = true;
        }
